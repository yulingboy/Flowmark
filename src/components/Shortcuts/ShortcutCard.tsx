...
export function ShortcutCard({ item, onClick, className = '' }: ShortcutCardProps) {
  ...
  return (
    <>
      <button
        onClick={handleClick}
        className={`flex flex-col items-center gap-1 cursor-pointer group w-full h-full relative ${className}`}
      >
        {/* 打开方式标识 */}
        <div className="absolute -top-1 -right-1 z-10">
          {isPopupMode ? (
            <div className="w-4 h-4 rounded-full bg-purple-500 flex items-center justify-center shadow" title="弹窗打开">
              <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
              </svg>
            </div>
          ) : (
            <div className="w-4 h-4 rounded-full bg-blue-500 flex items-center justify-center shadow" title="新标签页打开">
              <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
